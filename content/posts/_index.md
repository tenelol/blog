+++
title = "Writing"
date = 2026-04-18T00:00:00+09:00
draft = false
description = "このサイト内の公開メモと、Zenn に公開している技術記事を同じ流れで読める一覧。"
+++

このサイトでは AI を使った試行、個人的な公開メモ、設計メモを扱います。自分で書いた技術的な記事は Zenn に寄せ、その導線もここにまとめます。コードだけでなく、考え方や改善の跡も見てもらうためのセクションです。

## Featured On Zenn

- [NixOSをCloudflare Tunnel経由でサーバー化した話](https://zenn.dev/tenelol/articles/417a294de03c5c)
- [NixOSをTailscale経由でリモートデプロイ可能にした話](https://zenn.dev/tenelol/articles/91672b444d6547)
